...
class TaskPolicy
  ...
  # The show policy check is invoked when we call `authorize @task`  from the show action of tasks controller.
  # Here the condition we want to check is that whether the record's creator is current user or record is assigned to the current user.
  def show?
